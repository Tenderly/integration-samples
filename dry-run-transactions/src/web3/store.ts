...
export const store = async (ownerAddress: string, value: string) => {
  ...
  try {
    const resp = await simulateTransaction(ownerAddress, storeContract, "store", +value)
    if (resp.data.simulation.status == false) {
        return {
            status: "Preventing failure in dryrun process"
        }
    }

    const tx = await storeContract.store(+value)
